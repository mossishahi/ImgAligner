...
def plot_feature_by_setting(df, show=True):
    ...
    for ax, col in zip(axes, group_cols):
        ...
        ax.set_title(f"Number of features per {col}")
        ax.set_xlabel(col)
        ax.set_ylabel("Count")
...
def plot_matches_scatter(df, 
    ...
    Parameters:
    - df: DataFrame with columns x1_roi, y1_roi, and the metadata column to color by
    ...
    # If color_by is numeric, use a colormap; otherwise, assign colors by category
    if pd.api.types.is_numeric_dtype(df[color_by]):
        sc = plt.scatter(df["x1_"+tag], df["y1_"+tag], c=df[color_by], cmap="viridis", s=10, alpha=0.7)
        plt.colorbar(sc, label=color_by)
    else:
        categories = df[color_by].unique()
        for cat in categories:
            subset = df[df[color_by] == cat]
            plt.scatter(subset["x1_"+tag], subset["y1_"+tag], s=10, alpha=0.7, label=str(cat))
        plt.legend(title=color_by)

    plt.gca().invert_yaxis()  # If image coordinates
    plt.xlabel("x1_"+tag)
    plt.ylabel("y1_"+tag)
    plt.title(title)
    plt.tight_layout()
...
def plot_matches_vectors(df,image=None, title="Matches vectors colored by angle",color='angle',cmap='hsv',tag='roi'):
    """
    Scatter plot of matched points with arrows representing vectors,
    colored according to their direction (radial angle).

    Parameters:
    - df: DataFrame with x1_roi, y1_roi, dx, dy
    """
    # Compute angle (radial axis)
    df["angle"] = np.arctan2(df["dy"], df["dx"])  # -pi to pi

    plt.figure(figsize=(10, 8))

    if image is not None:
        plt.imshow(image,cmap='Greys')
    # Quiver plot, color by angle
    q = plt.quiver(
        df["x1_"+tag], df["y1_"+tag],
        df["dx"], df["dy"],
        df[color],  # color by angle
